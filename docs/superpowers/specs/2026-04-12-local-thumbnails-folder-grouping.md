...
시스템에 설치된 FFmpeg CLI를 호출하는 방식.

**확인 시점:** 앱 시작 시 + 스캔 시작 시

**확인 방법:**
- `Command::new("ffmpeg").arg("-version")` 실행
- exit code 0이면 사용 가능

**FFmpeg 없을 때:**
- 스캔 자체는 정상 동작 (썸네일 생성만 스킵)
- 프론트엔드에 `ffmpegAvailable: boolean` 상태 전달
- 스캔 완료 후 "FFmpeg가 설치되지 않아 썸네일을 생성할 수 없습니다" 안내 표시
